...
def concatenate():
    ...
    # Group the cycle information by base_name for processing
    ...
    for base_name, group_df in base_name_groups:
        ...
        # Dictionary to collect all channels for each sequence
        sequence_channels = {}
        
        # First pass: collect all channels for each sequence
        for _, row in group_df.iterrows():
            ...
            try:
                ...
                channel_paths.sort(key=lambda x: x['channel_int'])
                
                # Store all channels for this sequence
                sequence_channels[seq_num] = channel_paths
                print(f"Sequence {seq_num} has {len(channel_paths)} channels")
                
            except FileNotFoundError:
                print(f"Warning: Path {path} not found, skipping")
        
        # If we have channels for at least one sequence
        if sequence_channels:
            ...
            # For each channel position, create a group across all sequences
            for pos_idx in range(max_channels):
                # Collect channels at this position from each sequence
                channel_group = []
                
                # Get all sequences in order
                seq_nums = sorted(sequence_channels.keys())
                
                # For each sequence, get the channel at this position
                for seq_num in seq_nums:
                    channels = sequence_channels[seq_num]
                    if pos_idx < len(channels):
                        channel_group.append(channels[pos_idx])
                
                if not channel_group:
                    continue
                
                # Create a name for this merged group using the actual channel IDs
                channel_ids = '_'.join(ch['channel_id'] for ch in channel_group)
                group_key = f"Merged_Pos{pos_idx+1}_Ch{channel_ids}"
                safe_group_key = sanitize_filename(group_key)
                
                print(f"\nProcessing channel position {pos_idx+1}: {group_key}")
                ...
                if all_data:
                    ...
                    try:
                        ...
                        fallback_filename = os.path.join(output_dir, f"group_pos{pos_idx+1}_merged_cycles.csv")
                        try:
